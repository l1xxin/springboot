<template>
  <div>
	...
	<el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">
	  ...
	  <el-table-column label="操作"  width="600" align="center">
	    <template slot-scope="scope">
		  ...
		  <el-button type="primary" @click="addStu(scope.row.id)">添加学生 <i class="el-icon-edit"></i></el-button>
		  <el-button type="primary" @click="addTea(scope.row.id)">添加教师 <i class="el-icon-edit"></i></el-button>
	    </template>
	  </el-table-column>
	</el-table>
	<!-- 分页 -->
	...
	<el-dialog title="未加入活动学生列表" :visible.sync="dialogStuVisible" width="40%" :close-on-click-modal="false">
	  <!-- 学生表格 -->
	  <el-table :data="StuData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleStuSelection">
	    <el-table-column type="selection" width="55"></el-table-column>
...
export default {
  ...
  methods: {
	...
	handleStuSelection(val){
		console.log(val)
		this.stuSelection = val.map(({name,num})=>{
			return{
				userId:num,
				actId:this.act
			}
