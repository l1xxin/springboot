...
public class TaskViewController {
    ...
    // 新增或者更新
    @PostMapping
    public Result save(@RequestBody TaskView taskView) {
        if (taskView.getId() == null) {
            //taskView.setTime(DateUtil.now());
            //taskView.setUser(TokenUtils.getCurrentUser().getUsername());
        }
        taskViewService.saveOrUpdate(taskView);
        return Result.success();
    }

    @DeleteMapping("/{id}")
    public Result delete(@PathVariable Integer id) {
        taskViewService.removeById(id);
        return Result.success();
    }

    @PostMapping("/del/batch")
    public Result deleteBatch(@RequestBody List<Integer> ids) {
        taskViewService.removeByIds(ids);
        return Result.success();
    }

    @GetMapping
    public Result findAll() {
        return Result.success(taskViewService.list());
    }

    @GetMapping("/{id}")
    public Result findOne(@PathVariable Integer id) {
        return Result.success(taskViewService.getById(id));
    }
...
//        }
    ...
    */
    ...
    public void export(HttpServletResponse response) throws Exception {
        // 从数据库查询出所有的数据
        List<TaskView> list = taskViewService.list();
        // 在内存操作，写出到浏览器
        ...
        String fileName = URLEncoder.encode("TaskView信息表", "UTF-8");
    ...
    public Result imp(MultipartFile file) throws Exception {
        ...
        List<TaskView> list = reader.readAll(TaskView.class);
